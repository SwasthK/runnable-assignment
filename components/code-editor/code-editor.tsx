...
export default function CodeEditor({
  ...
  const el = node && node.type !== "text" ? (node as TreeElementNode) : null
  const classes = el?.props.className?.split(" ") ?? []
  const currentSize = classes.find((c) => FONT_SIZES.includes(c)) ?? ""
  const currentWeight = classes.find((c) => FONT_WEIGHTS.includes(c)) ?? ""
  const currentColor = (el?.props.style?.color as string) ?? "#000000"
  const currentBg = (el?.props.style?.backgroundColor as string) ?? "#ffffff"

  return (
    <>
      <div
        className={cn(
          "z-20 flex flex-col overflow-hidden border-l bg-background transition-all duration-300",
        ...
        <div className="flex-1 overflow-auto">
          {!node ? (
            ...
            <div className="space-y-4 p-4">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        updateNode(node.id, (n) => {
                          ...
                          el.props.className = updateClass(
                            el.props.className,
                            "text-",
                            value
                          )
                ...
                <div className="">
                  <div className="flex flex-col gap-1">
                    ...
                    <Input
                      ...
                      type="color"
                      value={currentColor}
                      className="border-md h-8 w-8 cursor-pointer p-0.5"
                      onChange={(e) =>
                        updateNode(node.id, (n) => {
                          const el = n as TreeElementNode
                          el.props.style = {
              ...
              <div className="space-y-2">
                <Label className="tracking-wide">Background</Label>
                <div className="flex flex-col gap-1">
                  <Label
                    htmlFor="bg-color"
                    className="text-xs tracking-wide text-muted-foreground"
